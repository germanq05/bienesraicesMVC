...
describe('Prueba la Navegcion y Routing del Header y Footer', () => {

    it('Prueba la Navegacion del Header', () => {
        cy.visit('/');

        cy.get('[data-cy="navegacion-header"]').should('exist');
        cy.get('[data-cy="navegacion-header"]').find('a').should('have.length', 4); //Debe haber 4 enlaces
        cy.get('[data-cy="navegacion-header"]').find('a').should('not.have.length', 3); //Debe haber 4 enlaces
    ...
    it('Prueba la Navegacion del Footer', () => {
        ...
        cy.get('[data-cy="navegacion-footer"]').find('a').should('have.length', 4); //Debe haber 4 enlaces
        cy.get('[data-cy="navegacion-footer"]').find('a').should('not.have.length', 3); //Debe haber 4 enlaces
